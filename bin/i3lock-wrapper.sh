...
params=$(getopt \
         -o vnbdI:up:efc:lh \
         -l version,nofork,beep,dpms,inactivity-timeout:,no-unlock-indicator,pointer:,ignore-empty-password,show-failed-attempts,color:,logo,help \
         -n "$script_name" -- "$@")
eval set -- "$params"
...
usage () {
    cat <<EOF
A wrapper around i3lock that sets a blurred image of the 
wallpaper as a background image for the screen saver.
...
Usage: $script_name [options]
    ...
    -e, --ignore-empty-password when an empty password is provided by the user,
                                do  not  validate  it.
    -c rrggbb,
    --color=rrggbb              turn the screen into the given color instead of
                                white. Color must  be  given in 3-byte format. 
           ...
           -c|--color)
               ...
               case "$1" in
                   [a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9])
                       shift;;
                    *)
                       echo "$script_name: invalid option \"$1\" for argument -c:" \
                            "3-byte RGB color expected" >&2
                       exit 1
                       ;;
               esac
               ;;
...
file1=$(cat ~/.fehbg | grep "/home/antcc" | cut -d' ' -f3 | cut -d"'" -f2)
file2=$(dirname $file1)/blurred-$(basename $file1)

if [ ! -f "$file2" ]
then
    convert "$file1" -blur 0x10 "$file2"
fi
